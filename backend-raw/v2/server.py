...
class omfg(object):
    ...
    def GET(self, var = None, code = '', user_id = None, date_time = None, time_from = None, time_to = None):
        ...
        if(var == 'all' and user_id is not None and time_from is not None and time_to is not None):
            ...
            return_data = dbhandler.gatherer(data, cnx, cursor)
            cnx.close()
            cursor.close()
            return return_data
    ...
    def POST(self, var = None):
        ...
        if(var == 'eyetrack'):
            try:
                ...
                returni = dbhandler.addEyetrackerdata(data, cnx, cursor)
        ...
        if(var == 'heartrate'):
            ...
            if('user_id' in data and 'datetime' in data):
                returni = dbhandler.populateHR(data, cnx, cursor)
                cursor.close()
                cnx.close()
                return returni
